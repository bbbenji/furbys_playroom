...
<template>
  <div class="mascot-container">
    ...
    >
      ...
      <div v-if="store.mascotMood === 'sleeping'" class="particles sleep-particles">
        <span class="zzz z1">Z</span>
        <span class="zzz z2">z</span>
        <span class="zzz z3">z</span>
      </div>

      <div v-if="store.mascotMood === 'singing'" class="particles music-particles">
        <span class="note n1">♪</span>
        <span class="note n2">♫</span>
        <span class="note n3">♬</span>
      </div>

      <div v-if="store.mascotMood === 'eating'" class="particles food-particles">
        <span class="crumb c1">✨</span>
        <span class="crumb c2">🍪</span>
        <span class="crumb c3">⭐</span>
      </div>

      <div v-if="store.mascotMood === 'farting'" class="particles fart-particles">
      ...
      >
        <defs>
          ...
          <filter id="furGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="4" stdDeviation="6" flood-color="#a855f7" flood-opacity="0.35" />
          </filter>
        </defs>

        <!-- Ears -->
        <g class="ear left-ear">
          <ellipse cx="44" cy="56" rx="22" ry="36" fill="#7e22ce" transform="rotate(-30 44 56)" />
          <ellipse cx="44" cy="56" rx="12" ry="24" fill="#f472b6" transform="rotate(-30 44 56)" />
        </g>
        <g class="ear right-ear">
          <ellipse cx="156" cy="56" rx="22" ry="36" fill="#7e22ce" transform="rotate(30 156 56)" />
          <ellipse cx="156" cy="56" rx="12" ry="24" fill="#f472b6" transform="rotate(30 156 56)" />
        </g>

        <!-- Main Body -->
        <circle cx="100" cy="115" r="72" fill="url(#bodyGrad)" filter="url(#furGlow)" />

        <!-- Tummy Patch -->
        <ellipse cx="100" cy="138" rx="42" ry="38" fill="url(#bellyGrad)" opacity="0.9" />
        ...
        <!-- Cheeks -->
        <circle cx="62" cy="106" r="8" fill="#f43f5e" opacity="0.45" class="blush" />
        <circle cx="138" cy="106" r="8" fill="#f43f5e" opacity="0.45" class="blush" />
        ...
        <g class="eyes-group">
          ...
          <!-- Normal / Active Eyes -->
          <g v-if="store.mascotMood !== 'sleeping' && store.mascotMood !== 'happy'" class="normal-pupils">
            <!-- Left Pupil -->
            <ellipse
              cx="78"
              cy="87"
              :rx="store.mascotMood === 'farting' || store.mascotMood === 'surprised' ? 14 : 10"
              :ry="store.mascotMood === 'farting' || store.mascotMood === 'surprised' ? 14 : 12"
              fill="#06b6d4"
            />
          ...
          <g v-else-if="store.mascotMood === 'happy'" class="happy-eyes">
            <path d="M 66 88 Q 78 72 90 88" stroke="#1e1b4b" stroke-width="4.5" fill="none" stroke-linecap="round" />
            <path d="M 110 88 Q 122 72 134 88" stroke="#1e1b4b" stroke-width="4.5" fill="none" stroke-linecap="round" />
          </g>

          <!-- Sleeping Eyes (- -) -->
          <g v-else-if="store.mascotMood === 'sleeping'" class="sleepy-eyes">
            <path d="M 66 88 Q 78 96 90 88" stroke="#1e1b4b" stroke-width="4.5" fill="none" stroke-linecap="round" />
            <path d="M 110 88 Q 122 96 134 88" stroke="#1e1b4b" stroke-width="4.5" fill="none" stroke-linecap="round" />
          </g>

          <!-- Eyelids (Blinking) -->
          <g class="eyelid-layer">
            <ellipse cx="78" cy="86" rx="19" ry="21" fill="#2e1065" class="eyelid eyelid-left" />
            <ellipse cx="122" cy="86" rx="19" ry="21" fill="#2e1065" class="eyelid eyelid-right" />
          </g>

          <!-- Eyelashes -->
          <path d="M 68 67 L 62 62" stroke="#4c1d95" stroke-width="2.5" stroke-linecap="round" />
          <path d="M 78 64 L 78 57" stroke="#4c1d95" stroke-width="2.5" stroke-linecap="round" />
          <path d="M 88 67 L 94 62" stroke="#4c1d95" stroke-width="2.5" stroke-linecap="round" />

          <path d="M 112 67 L 106 62" stroke="#4c1d95" stroke-width="2.5" stroke-linecap="round" />
          <path d="M 122 64 L 122 57" stroke="#4c1d95" stroke-width="2.5" stroke-linecap="round" />
          <path d="M 132 67 L 138 62" stroke="#4c1d95" stroke-width="2.5" stroke-linecap="round" />
        </g>
        ...
        <g class="beak-group">
          ...
          <ellipse
            v-if="store.mascotMood === 'eating' || store.mascotMood === 'singing' || store.mascotMood === 'talking'"
            cx="100"
            cy="114"
            rx="14"
            ry="11"
      ...
      <div class="tap-hint">
        ...
        <span v-else-if="store.mascotMood === 'farting'">Oops! Excuse me! 💨</span>
        ...
        <span v-else-if="store.mascotMood === 'talking'">Chit chat chat! 💬</span>
...
@keyframes glowPulse {
  ...
  100% { filter: drop-shadow(0 0 24px rgba(245, 158, 11, 0.85)); }
}
...
.z1 { right: 30px; top: 20px; font-size: 1.4rem; animation-delay: 0s; }
.z2 { right: 18px; top: -2px; font-size: 1.1rem; animation-delay: 0.7s; }
.z3 { right: 8px; top: -22px; font-size: 0.85rem; animation-delay: 1.4s; }

@keyframes floatZzz {
  0% { opacity: 0; transform: translateY(10px) scale(0.6); }
  50% { opacity: 1; }
  100% { opacity: 0; transform: translateY(-26px) translateX(12px) scale(1.2); }
}
...
.n1 { left: 24px; top: 15px; animation-delay: 0s; }
.n2 { right: 28px; top: 18px; animation-delay: 0.5s; }
.n3 { left: 45px; top: -10px; animation-delay: 1s; }

@keyframes floatNote {
  0% { opacity: 0; transform: translateY(8px) scale(0.7); }
  50% { opacity: 1; }
  100% { opacity: 0; transform: translateY(-30px) rotate(15deg) scale(1.1); }
}
...
.c1 { left: 30px; top: 40px; }
.c2 { right: 25px; top: 45px; animation-delay: 0.15s; }
.c3 { left: 50%; top: 10px; transform: translateX(-50%); animation-delay: 0.25s; }

.cl1 { right: 15px; bottom: 35px; animation: fartPuff 1.2s ease-out forwards; }
.cl2 { right: 5px; bottom: 50px; animation: fartPuff 1.2s ease-out 0.2s forwards; }

@keyframes fartPuff {
  0% { opacity: 0; transform: scale(0.5) translateX(-10px); }
  40% { opacity: 1; transform: scale(1.3) translateX(10px); }
  100% { opacity: 0; transform: scale(1.6) translateX(25px); }
}

.h1 { left: 25px; top: 20px; }
.h2 { right: 25px; top: 20px; animation-delay: 0.15s; }
.h3 { left: 50%; top: -15px; transform: translateX(-50%); animation-delay: 0.3s; }

@keyframes floatBurst {
  0% { opacity: 0; transform: scale(0.4) translateY(0); }
  40% { opacity: 1; }
  100% { opacity: 0; transform: scale(1.2) translateY(-25px); }
}
