...
type SymbolMsg = { symbol: string | null; magnitude: number; magnitudes: Record<string, number> }
...
export class ComAirReceiver {
  ...
  async start(): Promise<void> {
    if (!navigator.mediaDevices?.getUserMedia) {
      throw new DOMException('This browser has no microphone API', 'NotSupportedError')
    }
    ...
    try {
      ...
      if (!this.ctx.audioWorklet) {
        throw new DOMException('This browser has no AudioWorklet support', 'NotSupportedError')
      }
      ...
      this.node.port.onmessage = (event: MessageEvent<SymbolMsg>) => this.handleSymbol(event.data)
...
export function friendlyReceiverError(err: unknown): string {
  ...
  switch (name) {
    ...
    case 'NotSupportedError':
      return err instanceof Error && err.message ? err.message : "This browser doesn't support the audio features needed to listen."
    case 'AbortError':
